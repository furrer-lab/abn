...
forLoopContent_copy <-
  function(row.num,
           ...
           n,
           verbose) {
    # Case with group.var != NULL are not considered
...
# Modified version of the forLoopContent_copy function extracted from buildScoreCache.mle()
# cache_X and Y_list are passed as argument to the function, X and Y are still computed inside
forLoopContent_precomputedXY_all <- function(row.num,
                                             cache_X,
                                             Y_list,
                                             mycache,
                                             data.dists,
  ...
  if (exists(key, envir = cache_X, inherits = FALSE)) {
    ...
    obj <- compute_X(mycache$node.defn.multi[row.num, ], adj.vars, multinomial, data.df.multi)
    assign(key, obj, envir = cache_X)
    X <- obj
  }
  child <- mycache[["children"]][row.num]
...
# Modified function with pure matrix preallocation
# Precompute X and Y
# Note that: group.var is assumed to be NULL
call_forLoopContent_precomputedXY_all <- function(adj.vars,
                                          nvars,
                                          data.df,
                                          data.df.multi,
                                          data.df.lvl,
  ...
  res <- foreach(row.num = 1:rows,
                 .combine='rbind',
                 .packages = c("stats", "lme4", "mclogit", "nnet"),
                 .export = 'forLoopContent_precomputedXY_all',
                 .verbose = verbose) %do% {
                   forLoopContent_precomputedXY_all(row.num = row.num,
			          cache_X = cache_X,
		                  Y_list = Y_list,
                                  mycache = mycache,
                                  data.dists = data.dists,
