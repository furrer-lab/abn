...
```{r workflow, echo=FALSE, fig.cap="Workflow of the 2-steps inference procedure.",eval=TRUE,out.width="80%"}
...
$\mathbb{E}(X|\mathscr{A}) = \beta_{0} + \beta_{1} A1 +  \beta_{2} A2.$ 
    ...
    - the variance of $X$ given $\mathscr{A}$ is defined by the precision $\tau$ of the node, which is estimated during the model fitting process
$var(X|\mathscr{A}) = \frac{1}{\tau}.$
...
- if $X$ is multinomial:
    ...
    - the probability of $X=\ell$ (for a given category $\ell$ of $X$) given $\mathscr{A}$ is:
$\mathbb{P}(X=\ell|\mathscr{A}) = \frac{\exp \left( \beta_{0,\ell} + \beta_{1,\ell} A1 + \beta_{2,\ell} A2 \right)}{1+ \sum_{j=1}^{K-1} \exp \left( \beta_{0,j} + \beta_{1,j} A1 + \beta_{2,j} A2\right)}$.

    - the probability of $X=K$ (the last reference category) given $\mathscr{A}$ is:  $\mathbb{P}(X=K|\mathscr{A})=\frac{1}{1+ \sum_{j=1}^{K-1} \exp \left( \beta_{0,j} + \beta_{1,j} A1 + \beta_{2,j} A2\right)}.$ 
...
- If $X$ is binomial or multinomial, Bayes' theorem gives us: 
...
- if $X$ is multinomial, we have $X \sim Cat(p_{X,1},...,p_{X,K})$, where $p_{X,1},...,p_{X,K}$ are estimated in Step 1.
...
- if $B$ is Gaussian, $B \sim N(\mu_B,\sigma_B^2)$, with:
    - $\mu_B = \beta_{B0} + \beta_{B1} X+ \beta_{B2} C$, and
...
For nodes with no parents (i.e. root nodes in the graph), the distribution is directly estimated using the observed data:
  ...
  - observed frequencies for multinomial nodes.
...
For the remaining nodes, we use the estimated distributions of their parents as values in the formula, unless those parent nodes are evidence (i.e. observed). Specifically:
  ...
  - if a parent $A$ is multinomial, we marginalize over its possible values.
