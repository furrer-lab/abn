...
- if $B$ is Gaussian, $B \sim N(\mu_B,\sigma_B^2)$, with:
    ...
    - $\sigma_B = \frac{\sum_i \hat{\varepsilon}_i^2}{n-p},$
where $\hat{\varepsilon}_i$ represents the residuals, calculated as $\hat{\varepsilon} = B - (\beta_0 + \beta_1 A1 + \beta_2 A2)$, $n$ is the total number of observations, and $p$ is the number of variables in the model (in this case $p=2$). 
