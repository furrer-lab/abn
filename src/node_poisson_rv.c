...
Rprintf("############ Warning - Priors turned off - use only for checking mlik value! ################\n");
  ...
  build_designmatrix_pois_rv(dag,obsdata,priormean, priorsd,priorgamshape,priorgamscale,designmatrix,nodeid,storeModes);
  
  nDim=designmatrix->numparams+1; /** +1 for group precision **/
  ...
  gparams.designdata=designmatrix;
   ...
   gparams.inits_adj=inits[0];/** use [i] for two tries */ 
   
   generate_pois_rv_inits(myBeta,&gparams);
  
   /*g_pois_outer_R(nDim,myBeta->data,&gparams);*/
   ...
   /*Rprintf("step size iteration %d\n",i);*/
    ...
    gparams.finitestepsize=gsl_vector_get(finitefactors,i)*finitestepsize;
     ...
     lbfgsb(nDim, lmm, myBeta->data, lowerbounds, upperbounds, nbd, &gvalue, &g_pois_outer_R,
                      &rv_dg_pois_outer_R, &failcode, 
	              &gparams,
	              factr,
                      pgtol, &fncount, &grcount,
                      maxiters, msg, trace, nREPORT);
		      
    if(!failcode){dag->nodeScoresErrCode[nodeid]=0;break;}/** break out of for loop if no error as we are done **/	     
   
    /** to get to here we got a fail above so try different initial condition guess before going to next finite-stepsize **/
    gparams.inits_adj=inits[1];/** use different initial guess */ 
    generate_pois_rv_inits(myBeta,&gparams);
    failcode=0;/** reset*/
    lbfgsb(nDim, lmm, myBeta->data, lowerbounds, upperbounds, nbd, &gvalue, &g_pois_outer_R,
                      &rv_dg_pois_outer_R, &failcode, 
  ...
  if(!ModesONLY){/** only want modes so can skip the rest **/
   ...
   if(dag->hessianError[nodeid]!=DBL_MAX && dag->hessianError[nodeid]>max_hessian_error){
     ...
     s1 = gsl_min_fminimizer_alloc (T1);
      ...
      lower_f=compute_mlik_pois_brent(lower, &gparams);/** value at lower point **/
      upper_f=compute_mlik_pois_brent(upper, &gparams);/** value at higher point **/
       ...
       switch(status){  /** choose which type of node we have */
                     case GSL_SUCCESS:{    
		                     /** successful finite diff so now do final computation with the optimal step size **/
                                     ...
                                     rv_hessg_pois_outer(myBeta,&gparams, hessgvalues,finitestepsize,hessgvalues3pt);/** EDIT BACK to "finitestepsize" start with LARGEST STEPSIZE **/
...
/** **************************************************************************************************************/
/** **************************************************************************************************************/
double g_pois_outer_R (int Rn, double *betaincTauDBL, void *params) /*typedef double optimfn(int n, double *par, void *ex);*/
{
  int i,j;
...
const gsl_vector *priormean = designdata->priormean;
  ...
  /** first we want to evaluate each of the integrals for each data group **/ 
   ...
   /** NOTE: uncomment next line as useful for debugging as this should be the same as logLik value from lme4 */
    /*Rprintf("total loglike=%e at %f %f %f\n",term1,gsl_vector_get(betaincTau,0),gsl_vector_get(betaincTau,1),1/gsl_vector_get(betaincTau,2));*/
/*error("");*/
			 ...
			 double (* compute_mlik_nm_brent) (double finitestepsize, void *params), gsl_min_fminimizer *s1, double *finitestepsize,double *saverror, int errverbose)
{
  ...
  *finitestepsize=delta;/** --copy-- current delta to finitestepsize as finitstepsize will be changed in here **/
  /** could probably do the memory withouth alloc free in here but outside of function to avoid extra mallocs */
   F1.function = compute_mlik_pois_brent;
   F1.params = gparams;
